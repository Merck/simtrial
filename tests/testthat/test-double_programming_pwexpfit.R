test_pwexpfit <- function(Srv, intervals){
  time <- Surv[, "time"]
  status <- Surv[, "status"]
...
testthat::test_that("Validation passed for Situation 1",{
  ...
  testthat::expect_equal(pwexpfit(intervals=c(3,6,Inf)),pwexpfit(intervals=c(3,6,Inf)))
})
...
testthat::test_that("Validation passed for Situation 2",{
  Srv <- Surv(time = Ex2delayedEffect$month, event = Ex2delayedEffect$evntd)
  testthat::expect_equal(pwexpfit(intervals=c(3,6,6)),pwexpfit(intervals=c(3,6,6)))
})
...
testthat::test_that("Validation passed for Situation 3",{
  Srv <- Surv(time = Ex2delayedEffect$month, event = Ex2delayedEffect$evntd)
  testthat::expect_equal(pwexpfit(intervals=c(3,6,50)),pwexpfit(intervals=c(3,6,50)))
})
...
testthat::test_that("Validation passed for Situation 4",{
  Srv <- Surv(time = Ex2delayedEffect$month, event = Ex2delayedEffect$evntd)
  max <- max(Srv[,"time"]) 
  testthat::expect_equal(pwexpfit(intervals=c(max, max+3, max+3)),pwexpfit(intervals=c(max, max+3, max+3)))
})
