...
#     h7=exp(-h3/2);
#     if (r2==0) {
#       if (ro>0) bvfinal=pnorm(min(h1,h2))+bv*r3*h7;
#       if (ro<=0) bvfinal=max(0,pnorm(h1)-pnorm(h2))-bv*r3*h7;
#     }else {
#       
#       h6=abs(h1-h2);
#       h5=h6*h6/2;
...
#       aa=0.5-h3/8;
#       ab=3-2*aa*h5;
#       bv=0.13298076*h6*ab*pnorm(-h6)-exp(-h5/r2)*(ab+aa*r2)*0.053051647;
#       r1=r3*x;
#       rr=r1*r1;
#       r2=sqrt(1-rr);
...
#   p2=mvtnorm::pmvnorm(-Inf,c(1,1),c(0,0),rho,algorithm=GenzBretz(maxpts=50000,abseps=0.00001))
#   expect_equal(p1,p2[1],tolerance = 0.00001)
# })
# 
...
#   bvsum1=sum(bv1);
#   bv=bv-bvsum1;
#   if (ro>0) bvfinal=pnorm(max(h1,h2))+bv*r3*h7;
#   if (ro<=0) bvfinal=max(0,pnorm(h1)-pnorm(h2))-bv*r3*h7;
#   bvfinal
# }
# 
...
#   rr12=r12*x;
#   rr13=r13*x;
#   del=1-rr12*rr12-rr13*rr13-r23*r23+2*rr12*rr13*r23;
#   fac=sqrt(del);
#   rr122=1-rr12*rr12;
#   rr133=1-rr13*rr13;
#   f1=rr13-r23*rr12;
...
#   TV=w*exp((rr12*h12-h122)/rr122)/sqrt(rr122)*pnorm(hp1)*r12+w*exp((rr13*h13-h132)/rr133)/sqrt(rr133)*pnorm(hp2)*r13;
#   TV=sum(TV);
...
#   p2=mvtnorm::pmvnorm(-Inf,c(1,1,1),c(0,0,0),rho,algorithm=GenzBretz(maxpts=50000,abseps=0.00001))
#   expect_equal(p1[1],p2[1],tolerance = 0.00001)
# })
# 
# corr1=sqrt(344/550)
# corr2=sqrt(190/550)
...
# mvtnorm::pmvnorm(-Inf,upper,c(0,0,0),rho,algorithm=GenzBretz(maxpts=50000,abseps=0.0000001))[1]