...
test_that("Test 6: Milestone", {
  skip("Need to account for `se` which returns two values")
  ...
  expected <- data.frame(
    method = rep("milestone", 18L),
    parameter = rep(10, 18L),
    estimation = rep(
      c(
        0.16097092361893622, 0.1752731092436976, 0.12045851966961263,
        0.11738941400903585, 0.15355822418246762, 0.1519773404060174
      ),
      rep(c(2L, 4L), 3)
    ),
    se = c(
      0.03693587681297664, 0.03662189834863626, 0.034952703615152854,
      0.03484070894801079, 0.034952703615152854, 0.03484070894801079,
      0.03614098127448581, 0.035312669921649095, 0.035432630739150366,
      0.034912158581439694, 0.035432630739150366, 0.034912158581439694,
      0.035815727559287504, 0.03505127094114008, 0.03540131462139614,
      0.034738243333119145, 0.03540131462139614, 0.034738243333119145
    ),
    z = rep(
      c(
        9.252619142383594, 12.078380683791904, 5.565741269919053, 5.457930240636103,
        9.051772787302813, 9.054982526543846
      ),
      rep(c(2L, 4L), 3)
    ),
    analysis = rep(1:3, 3),
...
test_that("Test 7: MaxCombo (WLR-FH(0,0) + WLR-FH(0, 0.5))", {
  skip("Need to account for `z` which returns two values")
