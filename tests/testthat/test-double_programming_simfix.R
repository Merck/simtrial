...
testthat::test_that("test for events in the correct directions in timing_type=3 comparing to timing_type=2",{
  ...
  for (i in 1:nrow(tt3test)){
    if ((tt3test$duration[i]>tt2test$duration[i])&(tt3test$event[i]>=tt2test$event[i])) {ttvalue[i]=1 }
    else if ((tt3test$duration[i]<=tt2test$duration[i])&(tt3test$event[i]<=tt2test$event[i])) {ttvalue[i]=1 }
    else {ttvalue[i]=0}
  }
...
testthat::test_that("test for timing_type=4 outputs using timing_type 1 and 2 output",{
  ...
  for (i in 1:nrow(tt4test)){
    if (tt1test$duration[i]<tt2test$duration[i]) {tt4event[i]=tt2test$event[i]}
    else {tt4event[i]=tt1test$event[i]}
  }
...
testthat::test_that("test for timing_type=5 outputs using timing_type 2 and 3 output",{
  ...
  for (i in 1:nrow(tt5test)){
    if (tt2test$duration[i]<tt3test$duration[i]) {tt5event[i]=tt3test$event[i]}
    else {tt5event[i]=tt2test$event[i]}
  }
