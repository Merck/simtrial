test_that("rand_by_fixed_block returns an appropriate object: a vector of length n of treatment group",{
  s <- rand_by_fixed_block(n=10)
...
test_that("the final block is a partial block if `n` is not an exact multiple of the block length",{
  n <- 10
  block <- c(0,0,1,1)
  s <- rand_by_fixed_block(n=n, block=block)
