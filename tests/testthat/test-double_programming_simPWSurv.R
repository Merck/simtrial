...
testthat::test_that("Strata percentage calculated from simulated dataset must be within the tolerance=0.002 of strata in setup (0.4,0.6)",{
  expect_equal(object=c(sum(stringr::str_count(x$Stratum, "Low"))/400000,sum(stringr::str_count(x$Stratum, "High"))/400000), expected=c(0.4, 0.6), tolerance=0.002) 
})

testthat::test_that("block calculated from simulated dataset equals size of 4 with 1:1 randomization, which is 2 for each arm",{
  expect_equal(object=bktest1, expected=rep(2,length(bktest1)))
  expect_equal(object=bktest2, expected=rep(2,length(bktest2)))             
})

testthat::test_that("failRates calculated from simulated dataset must be within the tolerance=0.015 of failRates in setting",{
  expect_equal(object=ratetest, expected=failRates$rate, tolerance=0.015)})

testthat::test_that("DropoutRates calculated from simulated dataset must be within the tolerance=0.0005 of DropoutRates=0.001 in setup",{
  duration=300
  drtest<-0
  for (i in 1:duration){
    drtest[i]=sum(x$dropoutTime<=i&x$dropoutTime>(i-1))/400000
  }
  expect_equal(object=drtest, expected=rep(0.001,300), tolerance=0.0004)})

testthat::test_that("enrollRates calculated from simulated dataset must be within the relative tolerance=0.05 of enrollRates in setup",{
  duration=300
  entest<-0
  for (i in 1:duration){
