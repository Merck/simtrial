---
title: "Improve the computational efficiency of `simPWSurv` and `rpwexp` "
...
bm_rpwexp <- function(n, nr, itr = 1000) {
  ...
  for (i in 1:itr) {
    ...
    end_time <- Sys.time()
    times[5] = times[5] + difftime(end_time, start_time, units = "secs")[[1]]
  }
...
df_times_n <- data.frame(x = times_n$n,
                         ...
                         group = c(rep("Naive method in R", nrow(times_n)),
                                   ...
                                   rep("Inverse CDF with exp in R", nrow(times_n)),
...
df_times_nr <- data.frame(x = times_nr$n_rates,
                          ...
                          group = c(rep("Naive method in R", nrow(times_nr)),
                                    ...
                                    rep("Inverse CDF with exp in R", nrow(times_nr)),
