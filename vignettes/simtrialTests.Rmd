...
# devtools::install_github("Merck/gsdmvn", ref = "issue-06-development")
library(gsdmvn)
library(gsDesign2)
library(gsDesign)
library(tibble)
...
# save data
saveData = FALSE
if (saveData) dataPath = "/Users/yang/Documents/Merck2022/simtrial_check/"
```
                    ...
                    seed = NULL)
{
  ...
  start <- Sys.time()
  set.seed(seed)
  xx_FH <- simtrial:::simfix_(nsim = nsim,
                   # sample size
  ...
  start <- Sys.time()
  xx_FH_new <- simtrial::simfix(nsim = nsim,
                  # sample size
                  sampleSize = x_FH$analysis$N,
                  # targeted total event count
                  targetEvents = x_FH$analysis$Events,
...
result_FH_10000_1 <- data.frame(
  method = c("Old simfix", paste0("New simfix w/ ", ncore, " cores")),
                    ...
                    )
{
  ...
  start <- Sys.time()
  set.seed(seed)
  xx_MC <- simtrial:::simfix_(nsim = nsim,
                 # sample size
                    ...
                    tau = 4)
{
  ...
  xx_MB <- NULL
  for (sim in 1:nsim) {
    xx_MB_n <- simtrial:::simPWSurv_(n = x_MB$analysis$N,
                           strata = tibble::tibble(Stratum = "All", p = 1),
