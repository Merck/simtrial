...
- 2 periods in failure rates of the control arm per stratum
- 2 periods in failure rates of the experimental arm per stratum
- 2 periods in dropout rates of the control arm per stratum
- 2 periods in dropout rates of the experimental arm per stratum
...
runSimfixVarCores <- function(
  ...
  times[1] <- difftime(Sys.time(), start, units = "min")[[1]]
  ...
  for (i in 2:length(ncores)) {
    ...
    times[i] <- difftime(Sys.time(), start, units = "min")[[1]]
    doParallel::stopImplicitCluster()
...
runAndPlotSimfixVarCores <- function(
  ...
  # Run simfixNew in serial and in parallel
...
runAndPlotSimfixVarCoresVarSims <- function(
  ...
  nsims <- sort(nsims)
  times_all <- rep(0, length(ncores) * length(nsims))
  sims_all <- rep(0, length(ncores) * length(nsims))
  labels_all <- character(length = length(ncores) * length(nsims))
  for (i in 1:length(nsims)) {
    s1 = (i-1) * length(ncores) + 1
    e1 = i * length(ncores)
    ...
    sims_all[s1:e1] <- rep(nsims[i], length(ncores))
  ...
  # Natural sorting so it's 1, 2, 4, 8, 16 instead of 1, 16, 2, 4, 8
  df_times$group <- factor(df_times$group, levels = stringr::str_sort(unique(df_times$group), numeric = TRUE))
  ...
  df_times$group <- as.character(df_times$group)
...
# Run simfix in serial and in parallel
testFix <- runAndPlotSimfixVarCores(nsim = n_sim, ncores = n_cores, seed = 2022)
```
...
### 10,000 to 25,000 simulations

```{r varsims}
n_sims <- c(10, 15, 20, 25) * 1000
n_cores <- c(1L, 2L, 4L)

# Run simfix in serial and in parallel
testVar <- runAndPlotSimfixVarCoresVarSims(nsims = n_sims, ncores = n_cores, seed = 2022)
```
