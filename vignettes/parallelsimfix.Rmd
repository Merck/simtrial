...
### Function for running simfix and simfixNew across varying number of cores
```{r runSimfixVarCores}
...
runAndPlotSimfixVarCores <- function(
  ...
  times_new <- runSimfixVarCores(nsim = n_sim,
                             ncores = ncores,
                             runsimfixNew = TRUE,
                             sampleSize = sampleSize,
                             targetEvents = targetEvents,
  ...
  df_times<- data.frame(x = sort(ncores),
                              y = c(times,
                                    times_new),
                              fun = c(rep("simfix", length(n_cores)),
                                      rep("simfixNew", length(n_cores))))
  pl_times <- ggplot(df_times, aes(x, y, col = fun)) +
    geom_line() +
    scale_x_continuous(trans='log2', breaks = n_cores) +
    labs(x = "number of cores", y = paste0("time (sec)"),
         title = paste0(n_sim, " simulations")) +
    theme_mk() +
