...
```
Overall `doAnalysis` take $70\%$ of the runtime in one simulation, but notice that `doAnalysis` is called 3 times in this example.
...
Both `tensurv` and `cutData` take $12\%$ to $20\%$ of the runtime in one simulation, but `tensurv` may be called multiple times (3 times in this example).
