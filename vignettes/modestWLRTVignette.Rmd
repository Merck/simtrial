...
sample_size <- 2000
enrollDuration <- .0001
enroll_rate <- data.frame(duration = enrollDuration, rate = sample_size / enrollDuration)
fail_rate <- data.frame(
  stratum = "All",
  fail_rate = 0.25,
  dropout_rate = 0,
...
xx <- FHwn |>
  counting_process(arm = "experimental") |>
  fh_weight(rho_gamma = data.frame(rho = c(0, 0, .5), gamma = c(0, .5, .5)), return_corr = TRUE) |>
  mutate(p = pnorm(z))
xx
```
