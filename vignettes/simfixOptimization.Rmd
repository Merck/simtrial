...
### `simfix`
```{r prof_simfix_all}
times_simfix <- mbm_simfix(timingType = 1:5,
                              useSimfixNew = FALSE,
                              nstrata = nstrata,
                              minFollow = minFollow,
                              fr = fr,
...
### `simfix` with `simPWSurvNew`
```{r prof_simfixNew_all}
times_simfixNew <- mbm_simfix(timingType = 1:5,
                           nstrata = nstrata,
                           minFollow = minFollow,
                           fr = fr,
                           dr = dr)
kable(times_simfixNew)
```
...
We focus on three cases when `timingType` is a scalar (1, 2, or 3).
  ...
  - With a more efficient `simPWSurvNew`, `simPWSurvNew` is no longer the main bottleneck of `simfix`, while `doAnalysis` takes $50\%$ to $60\%$ of the runtime in one simulation.
