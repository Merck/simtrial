...
mbm_sub$expression[[1]] <- "dplyr::filter"
mbm_sub$expression[[2]] <- "base filter"
mbm_sub
# plot(mbm_sub)
```
...
tensurvNew <- function(x, txval){
    ...
    x <- x %>% group_by(Stratum) %>% arrange(desc(tte)) %>%
          mutate(one=1,
                 atrisk=cumsum(one),
                 txatrisk=cumsum(Treatment==txval)) %>%
          # Handling ties using Breslow's method
          ...
          dplyr::summarise(events=sum(event),
                           ...
                           atrisk=max(atrisk),
                           txatrisk=max(txatrisk))
    x[which(x$events > 0 & x$atrisk > x$txatrisk & x$txatrisk > 0),] %>%
          select(-mtte) %>%
          mutate(s=1-events/atrisk) %>%
          arrange(Stratum, tte) %>%
          group_by(Stratum) %>%
...
bench::mark(
  d1 %>% simtrial::tensurv(txval="Experimental"),
  d1 %>% tensurvNew(txval="Experimental"),
  iterations = 20
)

bench::mark(
  d2 %>% simtrial::tensurv(txval="Experimental"),
  d2 %>% tensurvNew(txval="Experimental"),
  iterations = 20
)

bench::mark(
  d3 %>% simtrial::tensurv(txval="Experimental"),
  d3 %>% tensurvNew(txval="Experimental"),
  iterations = 20
)
```
