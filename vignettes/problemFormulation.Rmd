...
x2 <- x %>% arrange(Stratum, Treatment)
x3 <- x %>% arrange(Stratum, Treatment)
...
system.time({
  ...
  for (sr in usr) {
    for (tr in utr) {
      # set.seed(2022)
      indx <- x1$Stratum==sr & x1$Treatment==tr
      x1$failTime[indx] <- rpwexp(n=sum(indx),failRates=filter(failRates,Stratum==sr&Treatment==tr))
      x1$dropoutTime[indx] <- rpwexp(n=sum(indx),failRates=filter(dropoutRates,Stratum==sr&Treatment==tr))
    }
  }
})

# Function to parallel
generateTimes <- function(sr, tr) {
  # set.seed(2022)
  indx <- x3$Stratum==sr & x3$Treatment==tr
  output <- tibble(failTime = rpwexp(n=sum(indx), 
                                     failRates=filter(failRates,
                                                      Stratum==sr&Treatment==tr)),
                   dropoutTime = rpwexp(n=sum(indx),
                                        failRates=filter(dropoutRates,
                                                         Stratum==sr&Treatment==tr)))
  return(output)
}
...
system.time({
  ...
  for (sr in usr) {
    for (tr in utr) {
      indx <- x2$Stratum==sr & x2$Treatment==tr
      x2[indx, c("failTime", "dropoutTime")] <- generateTimes(sr, tr)
    }
  }
})

# Parallel code (TODO unlist results and write to x)
system.time({
  x3$failTime <- 0
  x3$dropoutTime <- 0
  test <- mcmapply(generateTimes,
                   rep(usr, each = length(utr), times = 1),
                   rep(utr, each = 1, times = length(usr)),
                   mc.cores = 8)
})
```
...
### The inverse transform method using `rexp(1)`
```{r}
...
### The inverse transform method using `runif` (as in `PWEALL`)
```{r}
rpwexpunif <- function(n, rate, t){
  # Need to check rate, t are positive real vectors of same length, t is increasing, h > 0
  n_piece <- length(rate)
  # Get number of piecewise rates
  H <- c(0, cumsum(rate * (t - dplyr::lag(t, default = 0))))[1:n_piece]
  t <- c(0, t)[1:n_piece]
  # Generate cumulative hazard for each observation
  y <- -log(runif(n))
  # Make a vector to record with interval each observation is in
...
set.seed(2)
PWEALL::rpwe(n = 10, rate = rate, tchange = tchange0)
set.seed(2)
rpwexpunif(n = 10, rate = rate, t = tchange1)
```
...
mbm2 <- microbenchmark(
  ...
  rpwexpRcpp(n = x_n, failRates = test_failRates),
  # Inverse exp(1)
  rpwexph(n = x_n, rate = test_failRates$rate, t = tchange1),
  # Inverse unif
  rpwexpunif(n = x_n, rate = test_failRates$rate, t = tchange1),
  # PWEALL
  PWEALL::rpwe(n = x_n, rate = test_failRates$rate, tchange = tchange0),
  times = 1000
)
levels(mbm2$expr) <- c("rpwexp", "rpwexpRcpp", "rpwexph", "rpwexpunif", "PWEALL")
mbm2
```
...
bm_rpwexp <- function(n, nr, itr = 1000) {
  ...
  for (i in 1:itr) {
    ...
    start_time <- Sys.time()
    rpwexph(n = n, rate = rate, t = tchange1)
    end_time <- Sys.time()
    times[3] = times[3] + end_time - start_time

    start_time <- Sys.time()
    rpwexpunif(n = n, rate = rate, t = tchange1)
    end_time <- Sys.time()
  ...
  return (times * 1000 / itr)
}

nr_pool <- c(2, 4, 6, 8, 10, 20, 50, 100) # number of time intervals
n_pool <- c(100, 200, 500, 1000, 1500, 2000, 5000, 10000) # number of observations to be generated
...
times_n <- data.frame(matrix(ncol = 6, nrow = length(n_pool)))
...
for (i in 1:length(n_pool)) {
  times_n[i, 2:6] <- bm_rpwexp(n_pool[i], nr)
}
...
df_times_n <- data.frame(x = times_n$n,
                         y = c(times_n$rpwexp,
                               ...
                               times_n$rpwexpunif,
                               times_n$PWEALL),
                         group = c(rep("rpwexp", nrow(times_n)),
                                   rep("rpwexpRcpp", nrow(times_n)),
                                   rep("rpwexph (inverse)", nrow(times_n)),
                                   rep("rpwexpunif", nrow(times_n)),
                                   rep("PWEALL", nrow(times_n))))
ggplot(df_times_n, aes(x, y, col = group)) +
  geom_line() +
  scale_x_log10(breaks = c(100, 200, 500, 1000, 2000, 5000, 10000)) +
  # scale_x_continuous(trans='log10') + 
  scale_y_continuous(trans='log10') + 
  labs(x = "number of observations to be generated", y = "time (ms)")
```
...
# Dataframe for storing timing results
times_nr <- data.frame(matrix(ncol = 6, nrow = length(nr_pool)))
...
for (i in 1:length(nr_pool)) {
  times_nr[i, 2:6] <- bm_rpwexp(n, nr_pool[i])
}
...
df_times_nr <- data.frame(x = times_nr$n_rates,
                          y = c(times_nr$rpwexp,
                                ...
                                times_nr$rpwexpunif,
                                times_nr$PWEALL),
                          group = c(rep("rpwexp", nrow(times_nr)),
                                    rep("rpwexpRcpp", nrow(times_nr)),
                                    rep("rpwexph (inverse)", nrow(times_nr)),
                                    rep("rpwexpunif", nrow(times_nr)),
                                    rep("PWEALL", nrow(times_nr))))
ggplot(df_times_nr, aes(x, y, col = group)) +
  geom_line() +
  scale_x_continuous(trans='log10') +
  labs(x = "number of intervals", y = "time (ms)")
```
