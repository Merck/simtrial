...
for (sim in 1:n_sim) {
   ...
   # cut data for analysis
   # analyses would be performed when both populations reach targeted endpoints.
   y <- cutDataAtCount(x, 150)
   ten <- tensurv(y, txval = "Experimental")
   
   # generate a stratified (weighted) logrank test and a corresponding one-sided p-value
   ans <- rbind(ans, tenFH(x = ten, rg = tibble(rho = c(0, 0, 1, 1), gamma = c(0, 1, 0, 1))) %>% mutate(pnorm(Z), sim_ID = sim))
   
   #cum_event <- cum_event + (y %>% ungroup() %>% summarise(n = sum(event)))$n
}
```
