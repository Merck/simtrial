...
tchange <- cumsum(c(0, test_failRates$duration))[1:(length(test_failRates$duration-1))] # exclusive prefix sum of duration
...
mbm2 <- microbenchmark(
  ...
  rpwexpRcpp(n = x_n, failRates = test_failRates),
  # PWEALL
  rpwe(n = x_n, rate = test_failRates$rate, tchange = tchange),
  times = 1000
)
levels(mbm2$expr) <- c("rpwexp", "rpwexpRcpp", "PWEALL")
mbm2
```

## Time comparison of generating random variable from piecewise Exponential distribution
```{r}
bm_rpwexp <- function(n, nr, itr = 1000) {
  ...
  tchange <- cumsum(c(0, duration))[1:(length(duration-1))]
  ...
  for (i in 1:itr) {
    ...
    start_time <- Sys.time()
    rpwe(n = n, rate = rate, tchange = tchange)
    end_time <- Sys.time()
...
times <- data.frame(matrix(ncol = 4, nrow = length(n_pool)))
colnames(times) <- c("n", "rpwexp", "rpwexpRcpp", "inverse")
times$n <- n_pool

for (i in 1:length(n_pool)) {
  times[i, 2:4] <- bm_rpwexp(n_pool[i], nr)
}

df_times <- data.frame(x = times$n,                           
                       y = c(times$rpwexp, times$rpwexpRcpp, times$inverse),
                       group = c(rep("rpwexp", nrow(times)),
                                 rep("rpwexpRcpp", nrow(times)),
                                 rep("inverse", nrow(times))))
