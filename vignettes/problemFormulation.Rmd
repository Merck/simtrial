...
mbm2 <- microbenchmark(
  ...
  # Inverse unif in C++
  rpwexpinvRcpp(n = x_n, rate = test_failRates$rate, duration = test_failRates$duration),
  # PWEALL (Inverse unif in Fortran)
  PWEALL::rpwe(n = x_n, rate = test_failRates$rate, tchange = tchange0),
  times = 1000
)
levels(mbm2$expr) <- c("rpwexp", "rpwexpRcpp", "rpwexpexp", "rpwexpunif", "rpwexpinvRcpp", "PWEALL")
mbm2
```
...
bm_rpwexp <- function(n, nr, itr = 1000) {
  ...
  for (i in 1:itr) {
    ...
    start_time <- Sys.time()
    rpwexpinvRcpp(n = x_n, rate = rate, duration = duration)
    end_time <- Sys.time()
