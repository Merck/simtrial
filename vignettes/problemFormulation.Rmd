...
times <- data.frame(matrix(ncol = 6, nrow = length(n_pool)))
colnames(times) <- c("n", "rpwexp", "rpwexpRcpp", "rpwexph", "rpwexpunif", "PWEALL")
times$n <- n_pool

for (i in 1:length(n_pool)) {
  times[i, 2:6] <- bm_rpwexp(n_pool[i], nr)
}

df_times <- data.frame(x = times$n,                           
                       y = c(times$rpwexp, times$rpwexpRcpp, times$rpwexph, times$rpwexpunif, times$PWEALL),
                       group = c(rep("rpwexp", nrow(times)),
                                 rep("rpwexpRcpp", nrow(times)),
                                 rep("rpwexph (inverse)", nrow(times)),
                                 rep("rpwexpunif", nrow(times)),
