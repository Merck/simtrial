...
If we wanted to take the minimum of these for a MaxCombo test, we would first use `wlrcorr` to compute a correlation matrix for the above Z-statistics as follows.
...
```{r,message=FALSE}
x <- ten150 %>% wlrcorr(rg = tibble(rho = c(0, 0, 1, 1), gamma = c(0, 1, 0, 1)))
x %>% kable(digits = 2)
```
