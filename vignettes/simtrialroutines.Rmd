...
```{r}
fh_weight(x = ten150, rho_gamma = tibble(rho = c(0, 0, 1, 1), gamma = c(0, 1, 0, 1))) %>% kable(digits = 2)
```
...
```{r,message=FALSE}
x <- ten150 %>% fh_weight(rho_gamma = tibble(rho = c(0, 0, 1, 1), gamma = c(0, 1, 0, 1)), return_corr = TRUE)
x %>% kable(digits = 2)
```
