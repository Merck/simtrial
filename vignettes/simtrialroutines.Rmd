...
```{r}
rpw_enroll(n = 20, enroll_rate = data.frame(
  duration = c(1, 2),
  rate = c(2, 5)
))
```
...
```{r}
fh_weight(x = ten150, rho_gamma = data.frame(rho = c(0, 0, 1, 1), gamma = c(0, 1, 0, 1))) |> kable(digits = 2)
```
...
```{r,message=FALSE}
x <- ten150 |> fh_weight(rho_gamma = data.frame(rho = c(0, 0, 1, 1), gamma = c(0, 1, 0, 1)), return_corr = TRUE)
x |> kable(digits = 2)
```
